...
import { useDeleteTask, useTask } from "@/lib/hooks/use-tasks";
...
import { useDeleteRecurrenceRule } from "@/lib/hooks/use-recurrence-rules";
import { useRouter } from "next/navigation";

const TaskPage = ({ params }: { params: Promise<{ slug: string }> }) => {
  const { slug } = use(params);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteUpcomming, setDeleteUpcomming] = useState(false);
  ...
  const handleDelete = () => {
    deleteTask(task.id!);
    if (task.recurrenceRule && deleteUpcomming) {
      deleteRule({
        id: task.recurrenceRule.id!,
        deleteFutureTasks: deleteUpcomming,
      });
    }
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            <div className="w-full flex justify-between items-center">
              ...
              <div className="gap-4 flex items-center">
                <Button
                  variant="destructive"
                  disabled={isDeletingRule || isDeletingTask}
          ...
          <AlertDialogHeader>
            ...
            {task.recurrenceRule && (
              <div className="flex items-center gap-3">
                <Checkbox
                  id="terms"
                  checked={deleteUpcomming}
                  onCheckedChange={(value) =>
                    setDeleteUpcomming(Boolean(value))
                  }
                />
                <Label htmlFor="terms">Delete all upcoming tasks</Label>
              </div>
            )}
          </AlertDialogHeader>
          <AlertDialogFooter>
